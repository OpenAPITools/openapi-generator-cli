...
describe('VersionManagerController', () => {
  ...
  describe('commands', function () {
    ...
    describe('list', () => {
      ...
      it('has the options', () => expect(cmd.options).toEqual(
        [{flags: '-j, --json', description: 'print as json', defaultValue: false}]
      ))

      describe('the --json flag is set', () => {

        beforeEach(async () => {
          commandMock.refs['list [versionTags...]'].opts.mockReturnValue({json: true})
        ...
        it('prints the result as json', () => {
          expect(log).toHaveBeenNthCalledWith(1, JSON.stringify(versions, null, 2))
        })
      ...
      describe('the --json flag is not set', () => {
        ...
        beforeEach(async () => {
          ...
          versionManagerServiceMock.isSelectedVersion.mockImplementationOnce(v => v === '1.2.3')
          commandMock.refs['list [versionTags...]'].opts.mockReset().mockReturnValue({json: false})
          await cmd.action(['tag1', 'tag2']);
        })
        ...
        it('prints "No results for ..., if no versions are found', async () => {
          ...
          await cmd.action(['tag1', 'tag2']);
          expect(log).toHaveBeenNthCalledWith(1, chalk.red('No results for: tag1 tag2'))
        })
    ...
    describe('set', () => {
      ...
      it('sets version[0] from the list', async () => {
        ...
        await cmd.action(['tag1', 'tag2']);
        expect(versionManagerServiceMock.setSelectedVersion).toHaveBeenNthCalledWith(1, '1.2.3')
      })

      it('prints a message, if the search result is empty', async () => {
        versionManagerServiceMock.search.mockReturnValue(of([]))
        await cmd.action(['tag1', 'tag2']);
        expect(log).toHaveBeenNthCalledWith(1, chalk.red('Unable to find version matching criteria "tag1 tag2"'))
      })
